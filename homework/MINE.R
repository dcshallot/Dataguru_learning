...
# http://www.exploredata.net/ftp/MINE.r
# key index- MIC：the Maximal Information Coefficient

#方法概要：
# 用网格判断数据的集中程度，集中程度意味着是否有关联关系
# 方法具有一般性，即无论数据是怎样分布的，不限于特定的关联函数类型，此判断方法都是有效
# 方法具有等效性，计算的熵值和噪音的程度有关，跟关联的类型无关

#方法特性：
#1 如果变量对x,y存在函数关系，则当样本数增加时，MIC必然趋向于1
#2 如果变量对x,y可以由参数方程c(t)=[x(t),y(t)]所表达的曲线描画，则当样本数增加时，MIC必然趋于1
#3 如果变量对x,y在统计意义下互相独立，则当样本数增加时，MIC趋于0
